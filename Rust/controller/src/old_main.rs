	...
	'main: loop{
		...
		if let Some(true) = start{
			'manual: loop{
				...
				// test mode
// 				let test = (data.analog_stick_left.z == 1.0) && (data.analog_stick_right.z == 1.0);
// 				if test{
// 					'test: loop{
// 						let data = gamepad.read_data();
// 						let steering = data.analog_stick_left.x;
// 						let throttle = data.analog_stick_right.y;
// 						let throttle2 = data.analog_stick_right.x;
// 
// 						car.set_steering(90.0 + steering * 45.0)?;
// 						car.set_motor_test(throttle, throttle2)?;
// 						let exit = data.button_b;
