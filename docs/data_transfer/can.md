...
flowchart LR
  ...
  CAN_TX -->|tx_mutex_get(); read g_vehicle_speed| CAN_TX
  CAN_TX -->|make_speed_status_msg()| FDCAN_TX
  FDCAN_TX --> TRANS --> CANBUS --> PI
  PI -->|optional reply| CANBUS --> TRANS --> FDCAN_RX --> CAN_RX
  CAN_RX -->|ID==CMD_SPEED| Q_SPEED
  CAN_RX -->|ID==CMD_STEERING| Q_STEER
  CAN_RX -->|sets flag| EF

  %% Styling (Mermaid class/style)
  classDef hw fill:#f8f9fa,stroke:#c6cbd1;
  classDef threads fill:#eef6ff,stroke:#9fc5ff;
  classDef ipc fill:#fff4e6,stroke:#ffd7a8;
  class THREADS threads;
  class HW hw;
  class QUEUES ipc;
```

<!-- Mermaid sequence (compact) -->
```mermaid
sequenceDiagram
  autonumber
  participant Sensor as SpeedSensor
  participant TX as canTX
  participant FDCAN as FDCAN+Transceiver
  participant Bus as CAN_Bus
  participant Pi as RaspberryPi
  participant RX as canRX
  participant Qs as queue_speed_cmd
  participant Qv as queue_steer_cmd
  participant EF as EventFlags

  Sensor->>TX: signal FLAG_SENSOR_UPDATE
  TX->>TX: tx_mutex_get(); read g_vehicle_speed; tx_mutex_put()
  TX->>TX: make_speed_status_msg(msg)
  TX->>FDCAN: HAL_FDCAN_AddMessageToTxFifoQ(msg)
  FDCAN->>Bus: transmit frame (CAN_H/CAN_L)
  Bus->>Pi: Pi receives frame
  Pi->>Bus: (optional) reply
  Bus->>FDCAN: reply arrives
  FDCAN->>RX: ISR or poll -> HAL_FDCAN_GetRxMessage()
  alt CMD_SPEED
    RX->>Qs: tx_queue_send(queue_speed_cmd,msg)
    RX->>EF: tx_event_flags_set(FLAG_CAN_SPEED_CMD)
  else CMD_STEERING
    RX->>Qv: tx_queue_send(queue_steer_cmd,msg)
    RX->>EF: tx_event_flags_set(FLAG_CAN_STEER_CMD)
  else other
    RX->>RX: ignore / log
  end
```
